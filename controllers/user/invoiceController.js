...
const createInvoiceFromOrder = async (req, res) => {
  try {
    ...
    if (order.status !== 'delivered') {
      return res.status(400).json({ error: 'Invoice can only be generated after the order has been delivered' });
    }
...
const generateInvoicePdf = async (req, res) => {
  try {
    ...
    doc.pipe(res);
     ...
     currentY += 15;
     doc.fontSize(10).text('Floor No:4', leftColumnX, currentY);
     currentY += 15;
     doc.fontSize(10).text('Kannur, 670631', leftColumnX, currentY);
     currentY += 15;
     doc.fontSize(10).text('Phone: 9898900000', leftColumnX, currentY);
