...
exports.generateReport = async (req, res) => {
  ...
  if(!startDate || !endDate){
    req.flash('error', 'Both start date and end date are required.');
  ...
  try {
    ...
    if (type === "pdf") {
      console.log("iam in pdf");

      console.log("checkinggg pdf", orders);
      ...
      const startDate = orders.length
        ? moment(orders[0].orderDate).format("YYYY-MM-DD")
        : "N/A";
      const endDate = orders.length
        ? moment(orders[orders.length - 1].orderDate).format("YYYY-MM-DD")
        : "N/A";
      ...
      doc
        .fontSize(11)
        .text(`Date Range: ${startDate} - ${endDate}`, { align: "right" });
      doc.moveDown(2);

      if (orders.length === 0) {
        console.log("iam not getting the orders length");
        doc.moveDown(6);
        doc
          .fontSize(12)
          .text("No order details found for the selected date range.", {
            align: "center",
          });
        doc.end();
        return;
      ...
      orders.forEach((order) => {
        ...
        row.forEach((cell, i) => {
          doc
            .rect(currentLeft, currentTop, columnWidths[i], rowHeight)
            .stroke();
      ...
      doc
        .fontSize(11)
        .text(
          `Returned Orders: ${returnedOrdersCount}`,
          bottomLeft,
          summaryTop + 20
        )
        .text(
          `Returned Amount: Rs.${returnedAmount.toFixed(2)}`,
          bottomRight - 150,
          summaryTop + 20,
          { width: 150, align: "right" }
        );
      ...
      doc
        ...
        .text(
          `Net Total Amount: Rs.${netTotalAmount.toFixed(2)}`,
          bottomRight - 250,
          summaryTop + 40,
